      ...
      [
        {
          ...
          onPress: async () => {
            try {
              ...
              if (Platform.OS !== 'web' && RNFS.cacheDirectory) {
                try {
                  const cacheDir = RNFS.cacheDirectory;
                  const files = await RNFS.readDirectoryAsync(cacheDir);

                  // Delete cache files
                  for (const file of files) {
                    try {
                      await RNFS.deleteAsync(`${cacheDir}${file}`, { idempotent: true });
                    } catch (deleteError) {
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <AnimatedSection index={3}>
          ...
          <SettingItem
            ...
            onPress={() => navigation.navigate('/call-history')}
          />
        </AnimatedSection>
